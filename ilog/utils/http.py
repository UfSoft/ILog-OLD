...
"""
    ...
    :copyright: (c) 2010 by the ILog Team, see AUTHORS for more details.
...
from ilog.application import get_application, get_request, url_for
...
def check_external_url(app, url):
    ...
    """
    ilog_url = app.cfg['ilog_url']
    check_url = urljoin(ilog_url, url)
    if urlparse(ilog_url)[:2] != urlparse(check_url)[:2]:
...
def get_redirect_target(invalid_targets=(), request=None):
    ...
    for invalid in invalid_targets:
        if check_parts[:5] == urlparse(urljoin(ilog_url, invalid))[:5]:
            return
...
def make_external_url(path):
    ...
    return urljoin(get_application().cfg['ilog_url'], path.lstrip('/'))
...
def redirect_back(*args, **kwargs):
    ...
    log.debug("Initial redirect_back target: %r", target)
    if target is None:
        target = url_for(*args, **kwargs)
        log.debug("url_for built redirect_back target: %r", target)
    # call werkzeug's redirect directly and not the redirect() function
    # from this module because it will strip leading slashes this function
    # returns and thus generate wrong redirects.
