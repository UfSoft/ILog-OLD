...
def add_privilege(privilege):
    ...
    if privilege is None:
        privilege = ILOG_ADMIN
    elif privilege != ILOG_ADMIN:
        privilege = ILOG_ADMIN | privilege
    return privilege
...
def require_privilege(expr):
    """Requires ILOG_ADMIN privilege or one of the given."""
    def wrapped(f):
...
def privilege_attribute(lowlevel_attribute):
    ...
    def creator_func(privilege):
        ...
        priv = Privilege.query.filter_by(name=privilege.name).first()
        if priv is None:
